...
payments_stream = spark.readStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "pkc-lgwgm.eastus2.azure.confluent.cloud:9092") \
...
checkpoint_dir = "gs://spark_streaming_checkpoint/kafka_stream_checkpoint_mongo"

query = stateful_query.writeStream \
    .outputMode("append") \
    .format("mongodb") \
    .option('spark.mongodb.connection.uri', "mongodb+srv://kiran_db_username:dbpwd@mongo-db-cluster.saygd.mongodb.net/?retryWrites=true&w=majority&appName=mongo-db-cluster") \
    .option('spark.mongodb.database', 'ecommerce_mart') \
